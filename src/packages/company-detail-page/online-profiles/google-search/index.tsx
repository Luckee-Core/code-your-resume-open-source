          ...
          <div className={styles.actionButtons}>
            {googleSearchUrl ? (
              <a
                ...
                aria-label="Open Google search for company name in a new tab"
              >
              ...
              <button
                ...
                title="Add a company name (and optional notes) to open a Google search"
                aria-label="Add company name to search Google"
              >
                <ExternalLink className="h-3.5 w-3.5" aria-hidden />
              </button>
            )}
            <button
              ...
              title={
                isMissingRequiredWebsite
                  ? "Add a website on the company before discovering extra page URLs"
                  : playwrightDiscoveryAlreadyUsed
                    ? PLAYWRIGHT_WEBSITE_URL_DISCOVERY_RUN_DISABLED_TITLE
                    : "Discover extra page URLs from this site’s homepage links (same domain)"
              }
              aria-label="Discover site page URLs from homepage"
              onClick={async (e) => {
                e.preventDefault();
                e.stopPropagation();

                if (!company.id || !website || isSearching || playwrightDiscoveryAlreadyUsed) {
                  return;
                }

                setIsSearching(true);
                try {
                  const result = await dispatch(runCompanyDiscoverSitePageUrlsThunk(company.id));
                  if (!result.ok) {
                    if (result.error === "playwright_website_url_discovery_already_attempted") {
                      toast.error(
                        result.message ??
                          "Site page URL discovery can only be run once per company. Edit URLs manually if needed.",
                      );
                      return;
                    }
