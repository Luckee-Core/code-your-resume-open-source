...
/**
 * Generate an ideal-candidate answer and list matching graphics below.
 */
export const GenerateIdealCandidate = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (status === 200) {
        toast.success(
          "Ideal candidate generation started — it will appear below in a few minutes.",
        );
      } else if (status === 400) {
  ...
  return (
    <div className={styles.root}>
      <p className={styles.promptHint}>Answers: {IDEAL_CANDIDATE_QUESTION}</p>
      ...
      <JobDetailGraphicList
        jobId={job.id}
        kind="idealCandidate"
        emptyLabel="No ideal candidate answers for this job yet."
      />
    </div>
  );
};
