import { IMAGE_GRAPHICS_VAULT_KEY } from "@/config/local-storage-keys";
import type { ImageGraphic } from "@/model";

/** @deprecated Use IMAGE_GRAPHICS_VAULT_KEY from config/local-storage-keys */
export const IMAGE_GRAPHICS_STORAGE_KEY = IMAGE_GRAPHICS_VAULT_KEY;
...
export const readGraphicsVault = (): Vault => {
  ...
  return parseVault(window.localStorage.getItem(IMAGE_GRAPHICS_VAULT_KEY));
};

export const writeGraphicsVault = (graphics: ImageGraphic[]): void => {
  if (typeof window === "undefined") return;
  const vault: Vault = { graphics };
  window.localStorage.setItem(IMAGE_GRAPHICS_VAULT_KEY, JSON.stringify(vault));
};

/** Exposed for README / debugging; same key the API layer uses. */
export const getStorageKeyForDocs = (): string => IMAGE_GRAPHICS_VAULT_KEY;
