...
/**
 * POST /api/data/team-conversation/generate
 *
 * Queues team-conversation generation on Express; graphic is persisted server-side.
 *
 * @param input - Job id
 * @returns Accepted payload or structured error (never throws)
 */
export const generateTeamConversation = async (
  input: GenerateByJobIdInput,
): Promise<ApiResult<GenerateAcceptedResponse>> => {
  const result = await requestApi<GenerateAcceptedResponse>("/api/data/team-conversation/generate", {
    method: "POST",
