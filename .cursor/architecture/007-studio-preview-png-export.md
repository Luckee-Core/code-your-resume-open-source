...
| Piece | Role |
|--------|------|
| `src/packages/graphics-studio/builder-column/actions/index.tsx` | UI: **Download image** dispatches the thunk (full `preview` variant). |
| `src/packages/graphics-studio/builder-column/studio-thumbnail-download-panel/index.tsx` | UI: **Download thumbnail PNG** dispatches `{ variant: "thumbnail" }`. |
| `src/packages/graphics-studio/builder-column/send-off-sites-panel/index.tsx` | UI: outbound “send off” links (no Redux). |
...
5. **`scale`**  
   Default full export uses **`scale: 2`** for sharper PNGs on high-DPI displays. The **thumbnail** variant uses **`scale: 1`** and writes **`-thumbnail.png`** (smaller file, same capture pipeline).
...
- **Filename**: derived from `currentImageGraphic.title` or `id`, sanitized for download; suffix **`preview`** or **`thumbnail`** from thunk `variant`.
